...
module RSSGenerator
  class Generator < Jekyll::Generator
    def generate(site)
      # Read the YAML file
      websites = YAML.load_file(File.join(site.source, '_data', 'sites.yml'), permitted_classes: [Date])
      ...
      title.text = 'The Darktheme Club'
      channel.add_element(title)

      link = Element.new('link')
      link.text = 'https://darktheme.club'
      channel.add_element(link)

      description = Element.new('description')
      description.text = 'Updates for the Darktheme Club.'
      ...
      latest_websites.each do |website|
        ...
        description = Element.new('description')
        description.text = "Darktheme Club: #{website['domain']}"
